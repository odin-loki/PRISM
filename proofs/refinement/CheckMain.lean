...
function, then a summary line
  `summary agree=N agree-ext=N agree-reject=N outside=N mismatch=N`
(`agree`: the base fragment, `Sound.lean`; `agree-ext`: the extended
fragment — `freeze`, calls — `XSound.lean`).
...
def main (args : List String) : IO UInt32 := do
  if args.isEmpty then
    ...
    return 2
  let mut counts : Array Nat := #[0, 0, 0, 0, 0]
  for path in args do
    let text ← IO.FS.readFile path
    for r in parseFile text do
      let (v, msg) := checkAll r
      let k := match v with
        | .agree => 0 | .agreeReject => 1 | .outside => 2 | .mismatch => 3 | .agreeExt => 4
      counts := counts.modify k (· + 1)
      IO.println s!"{v.tag}\t{path}\t{r.name}\t{msg}"
  IO.println s!"summary agree={counts[0]!} agree-ext={counts[4]!} agree-reject={counts[1]!} outside={counts[2]!} mismatch={counts[3]!}"
