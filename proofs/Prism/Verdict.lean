...
def name : Stage → String
  ...
  | bmc => "bmc" | pir => "pir" | conc => "conc" | harness => "harness" | concolic => "concolic" | fuzz => "fuzz"
...
def origin : Stage → Origin
  ...
  | contracts | wp | bmc | pir | conc | harness | ltl => .solver
...
theorem proving_stages (s : Stage) :
    s.mayProve = true ↔ s ∈ [Stage.optional, .esbmc, .dafny, .contracts, .wp, .bmc, .pir, .conc, .harness, .ltl] := by
  revert s; decide
