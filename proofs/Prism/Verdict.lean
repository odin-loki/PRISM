...
def name : Stage → String
  ...
  | bmc => "bmc" | harness => "harness" | concolic => "concolic" | fuzz => "fuzz"
...
def origin : Stage → Origin
  ...
  | contracts | wp | bmc | harness | ltl => .solver
...
theorem proving_stages (s : Stage) :
    s.mayProve = true ↔ s ∈ [Stage.optional, .esbmc, .dafny, .contracts, .wp, .bmc, .harness, .ltl] := by
  revert s; decide
