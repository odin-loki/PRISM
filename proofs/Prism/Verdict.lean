...
inductive Stage where
  ...
  | dafny | contracts | wp | bmc | pir | conc | harness | review | concolic | fuzz | diff
...
def name : Stage → String
  ...
  | bmc => "bmc" | pir => "pir" | conc => "conc" | harness => "harness" | review => "review"
...
def origin : Stage → Origin
  ...
  | contracts | wp | bmc | pir | conc | harness | review | ltl => .solver
...
theorem proving_stages (s : Stage) :
    s.mayProve = true ↔ s ∈ [Stage.optional, .esbmc, .dafny, .contracts, .wp, .bmc, .pir, .conc, .harness, .review, .ltl] := by
  revert s; decide
