...
This is Helix's in-tree engine. Do not wrap the `frama-c` binary
...
from helix.contracts import bmc_function_with_assume, parse_comments
        ...
        if fn.kind == "POINTER" or body_needs_pointer_harness(fn.body or ""):
            out.append(Finding(
                **base, status=laws.NEEDS_HARNESS,
                ...
                extra={"engine": "helix-wp", "wp": "return-substitution"},
            ))
            continue
        if fn.kind != "SCALAR":
            out.append(Finding(
