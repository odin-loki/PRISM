"""Concrete interpreter of the Helix SCALAR C subset, with UB oracles.
...
def decode_args(fn: FunctionInfo, data: bytes) -> dict[str, int]:
    """Little-endian ints matching `fn.params` (Helix fuzzer harness layout)."""
    from helix.fuzz import C_TYPE_SIZE
...
# ----- statement / expression interpreter (same subset as helix.bmc) -----
...
class _Parser:
    ...
    def _assert(self, text: str) -> str:
        ...
        if not m:
            from helix.bmc import _paren_stmt
