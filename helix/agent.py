...
def _bmc_status_of_source(source: str, unwind: int = 2) -> str | None:
    ...
    Lazy-imports helix.bmc so unit tests that patch this (or never compile
    ...
    if not HAS_Z3:
        return None
    with tempfile.TemporaryDirectory(prefix="helix_rlef_bmc_", ignore_cleanup_errors=True) as td:
        p = Path(td) / "cand.c"
...
def sandbox_run(source: str, timeout: float = 8.0) -> dict:
    ...
    _silence_win_abort()
    with tempfile.TemporaryDirectory(prefix="helix_oci_", ignore_cleanup_errors=True) as td:
        src = Path(td) / "prog.c"
