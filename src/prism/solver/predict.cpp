...
// The model trained on this repository's logs and measured by
// tools/prism_ai/predict.py (docs/SOLVERS.md "Learned scheduler"),
// generated by `predict.py --emit-inc`; split into pieces so no single
// string literal exceeds compiler limits.
#include "predict_default.inc"

void validate(Loaded& e) {
...
const Loaded* load(const fs::path& cache_dir) {
    ...
    if (ec) {
        // No model file: the built-in one (a file, even a disabled one, overrides it).
        if (const char* m = std::getenv("PRISM_SOLVER_MODEL"); m && *m) return nullptr;
        static Loaded builtin = [] {
            Loaded e;
            std::string txt;
            for (const char* part : kDefaultModelParts) txt += part;
            try {
                e.j = json::parse(txt);
                validate(e);
            } catch (...) {
        ...
        return builtin.ok ? &builtin : nullptr;
    }
