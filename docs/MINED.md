...
| Clang AST (lints) | C++ engine only (roadmap 2.8, `src/prism/astlint*.cpp`). Front end: libclang's C API, loaded at run time (`astlint_libclang.cpp`, cursors converted to the `-ast-dump=json` schema), else `clang -ast-dump=json` per unit. Checks the unit, the project headers it includes, nested enums/members and non-dependent template code (dependent code stays unchecked). 54 classes on the AST (`astlint::CLASSES`), 46 of them ported from regex lints: assignment-as-condition, `sizeof` of a pointer/array param, signed/unsigned compare, `u < 0` (`CTRL-DEAD-GUARD`), enum switch hole, self-assignment/self-move, dead store, `memset` swap, int division to floating, shift/division UB, truncation, format count/type/width, `%n`, non-literal format, `strncpy` without NUL, fixed-size copy overflow, overlapping copy, `realloc` self-assign, fallthrough, shadowing, stack address / `c_str()` of a local returned, straight-line use-after-free, double free, mismatched free/delete, NULL deref in and after the NULL branch, uninitialised read, use after move, `[[nodiscard]]`, virtual call in ctor/dtor, missing virtual dtor, move of const, dangling ref, catch by value, throw in dtor/`noexcept`, `throw new`, `delete this`. The regex "return is discarded" family (466 functions, 260 classes) is ported as one table generated from the regex rules (`tools/gen_astlint_discard.py`). Supersedes the regex row on the same line/class; no libclang and no clang, or a parse error, is NOTRUN (`extra.layer`) and the regex lints still run |
