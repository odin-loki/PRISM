...
| Floating point in the `pir` stage (Z3's FP theory, FLOAT-CAST-OVF, `--fp-checks`) | Translation validation against `lli` with `--allow-exec`; `tests/pir/fp_*`; `tests/test_proofs_float_conc.py` locks the C++ condition text to the Lean definitions | **Proved in Lean** for the conditions (`proofs/refinement/PrismRefine/FloatOps.lean`): FLOAT-CAST-OVF is exactly C11's out-of-range condition, FLOAT-OVERFLOW is exactly IEEE overflow, FLOAT-INVALID is IEEE invalid on quiet operands, FLOAT-DIV-ZERO is IEEE divide-by-zero plus `0/0` and `∞/0`. Still trusted: Z3's FP theory implementing IEEE 754 (and rounding `Z3_mk_fpa_numeral_double` to nearest even), and the step from the C++ text to Z3 terms |
| Lazy-sequentialisation schedule of the `conc` stage | Conformance suite `tests/conformance/concurrency`, `tests/cpp/test_conc.cpp` | **Proved in Lean** (`proofs/techniques/PrismTechniques/LazySeqN.lean`): `K` rounds of `N` threads plus the harness's final slot is sound and covers every SC schedule of round-robin shape (every schedule with `≤ K−1` switches; `≤ 2K` for two threads). Still trusted: the Z3 slot formula in `lazy.cpp`, the race/deadlock monitors; not modelled: relaxed memory |
