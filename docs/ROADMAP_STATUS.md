...
| M8 | **PARTIAL** — k-induction, Houdini, contracts, bit-blaster proved; memory model proved in the semantics project; float partial. k-induction with a framed havoc (`KInduction.kinduction_frame_sound`, the pir step for memory-writing loops) is proved abstractly; the frame lemma for PIR memory (writes only inside the footprint, per-byte initialised flags) is not yet stated over `PrismSem/Memory.lean` |
