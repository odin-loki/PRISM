...
Headline numbers (C++ engine, strict `python tools/conformance.py` on the invariants merge; 328 true / 341 false tasks: in-house, SV-COMP subset, ESBMC C++ subset, libc-model and C++ library contracts, PIR round 3 and regression tasks):

| stage | wrong proofs | proved (true tasks) | refuted with replayed counterexample (false tasks) |
|---|---|---|---|
| bmc (hand encoder) | **0** | 102/328 | 93/341 |
| pir (Clang/LLVM front end) | **0** | 273/328 | 165/341 |
...
| 2.6 standard library | **PARTIAL** — libc operational models (`src/prism/pir/models/libc/`, contract-checked, 8.2); C++ model headers (`src/prism/pir/models/cxx/`, put first on the include path by the pir stage): `std::vector` (libstdc++ 13's capacity policy, allocation through the new/delete models so iterator invalidation is MEM-UAF, same exceptions and `_GLIBCXX_ASSERTIONS` preconditions); `std::map`/`multimap`/`set`/`multiset` (sorted node list, one heap node per element: erase-invalidated iterators are MEM-UAF, same order/uniqueness/hinted-multi-insert rule/construction points/exceptions, precondition checks for `end()`/`begin()`/foreign iterators; node handles and other allocators fall back to libstdc++); `std::string`'s out-of-line members (`bits/basic_string.tcc` model: element loops instead of symbolic-length memcpy, same SSO/growth/exceptions/messages; class and inline members stay libstdc++'s). All three are differential-tested against libstdc++ under ASan/UBSan (`tests/test_cxx_models.py`, identical traces). Translator: two-field aggregate returns (`std::pair<iterator,bool>`); `check_function` tries unwind 4 first and keeps only exact answers. Model build: own time limit, one retry, on-disk cache, NOTRUN per function if it still fails. Measured (single runs, loaded machine): `cxx_string_substr` true/false PROVED 1.7 s / FAILED 28 s (base: no answer), new map/set tasks 3/3 PROVED and 3/3 refuted (9–134 s), 0 wrong proofs. Value sets in the Bv memory encoding (`claude/w6-perf6`, docs/PIR.md "Encodings"): an input-dependent map erase followed by the destructor's traversal went from no answer in 900 s to PROVED in 2 s (UAF and wrong-sum twins FAILED); `map_contracts.cpp` 11/11 (7 PROVED, 4 refuted with the planted class) in 13–18 s as one file (it had also stopped compiling against the model headers with libstdc++ 14: `basic_string.tcc` fix); `vector_insert.cpp`: the UAF twin FAILED in 146 s, `vector_insert_true` still no answer in 1800 s (symbolic insert position). Not done / not measured: full esbmc-cpp + cxx before/after table; unique_ptr-under-load rerun not done; iostreams, unordered containers, libc++ (`_LIBCPP_HARDENING_MODE`) not modelled |
...
| 2.8 old encoder as differential oracle | **DONE** — `tools/pir_vs_bmc.py`; the old encoder is **not** retired yet (pir does not yet cover a superset). Front-end gaps closed in both engines (2026-09-24): structured `goto` (forward out of blocks/loops/switches, backward loops unwound and havocked like any loop; anything else NEEDS-HARNESS, no longer ERROR; `docs/SVCOMP.md` "goto"), function-try-blocks extracted (no PARSE-GAP; bmc NEEDS-HARNESS, pir models them), `<<` rules by language and `-std` (C/C++03, C++11–17, C++20; false alarm F7 fixed, `docs/CONFORMANCE.md`) |
...
| 3.1 learned scheduler | **DONE** — on by default (built-in GBDT, `PRISM_SOLVER_PREDICT=0` restores the rules; certified requests keep the rules). Trained on every member timed alone on 2,060 distinct pir VCs of `tests/pir`, `tests/conformance` (incl. esbmc-cpp, sv-comp) and `testdata`; held out by source file (599 VCs, 133 files). Replay at 2 cores: 13.0 s vs rules 17.3 s and history 15.9 s (noise 10–16%); real portfolio, two interleaved runs on 781 held-out VCs: 35.9 s vs 43.4 s rules / 41.7 s history (noise 0.5%), and 56.7 s vs 60.0 / 55.9 s in a run with one unexplained stall (not reproduced in a hunt of 8,960 portfolio calls under load; a watchdog now detaches a job that ignores its stop after 2 s, reaps a killed member in the background, and records every such event and every overrun with its phase times, `docs/SOLVERS.md` "Stalls"). 0 answer disagreements. The gain is Bitwuzla getting the head start (a static rule matches it); censored-loss GBDT, k-NN and a winner classifier measured, none better (`docs/SOLVERS.md` "Learned scheduler") |
...
| 8.2 LLVM IR → PIR refinement | **PARTIAL** — proved for the integer fragment (`proofs/refinement`), and, per function under a certificate the checker validates, for an extended fragment: `freeze`, `undef` under `freeze`/`store`, direct calls to same-file functions (inlined as `translate.cpp` does), stack memory (`alloca`, integer `load`/`store`, `getelementptr`, with the translator's null/wild/freed/bounds/alignment/read-only/uninitialised-read and pointer-arithmetic checks, on PRISM's object/offset memory model shared by both sides), the intrinsics `translate.cpp` translates specially (`smax`/`smin`/`umax`/`umin`, `abs`, `ctlz`/`cttz`, `ctpop`, `bswap`, `expect`, `*.with.overflow` + `extractvalue`, `lifetime.start`/`end`, `memcpy`/`memmove`/`memset` with constant or variable lengths) and globals the analysed function names (read-only data, `main`'s globals from their initialisers, other mutable/external globals as arbitrary bytes; the translator now allocates their variables before the function's results); per-run checker: `testdata` 915 `agree` + 32 `agree-ext` of 2 506 functions (was 21), `tests/pir` 46 + 29 of 228 (was 9), 0 mismatches. No theorem that `translateX` always yields a valid certificate (a proof about the stateful mirror; not attempted; `validB` shrunk where unused). Not in the fragment: library calls (1 330 `testdata` functions), `llvm.assume` (a false one is not reported: finding 8), heap, pointer parameters/phis/stored pointers, `undef` elsewhere. Findings 2, 5, 6, 7 fixed in the C++ translator (branch `claude/w6-falar`): `icmp samesign` is checked (UB-POISON), `freeze poison` is an arbitrary value (no report), `llvm.memcpy` may copy an object exactly onto itself (a C `memcpy` call keeps C's rule through its model: the pir stage compiles with `-fno-builtin-memcpy`), `llvm.lifetime.start` starts a stack object's lifetime again (PIR `revive`; any size); Lean: `cpyBad`/`overlapChk` changed and re-proved, `freeze poison` read as `freeze undef`, `lifetime.start` and `samesign` refused (outside), checker `testdata` 924 `agree` + 32 `agree-ext` of 2 515, `tests/pir` 47 + 26 of 231 (was 46 + 29 of 228: `mem_str.c` `memcpy_ok`, `memcpy_bad`, `overlap_bad` call C `memcpy`, which now reaches the library model, outside the fragment), 0 mismatches; `proofs/refinement/check.sh` OK (no `sorry`, three axioms). Refinement finding 4 fixed in the C++ translator: `undef` (and values computed from it) is a fresh value at every use, `br`/`noundef` on it is UB-POISON |
...
| 6.3 SV-COMP readiness | **PARTIAL** — BenchExec tool-info, benchmark definition, witness 2.0 writer (violation and correctness), scored subset (`docs/SVCOMP.md`). `true` answers carry an `invariant_set` correctness witness: `bmc` Houdini loop invariants at the loop keyword when the proof has them, else the empty (trivially valid) set; never an invariant the engine did not prove. A signed `<<` overflow counts as no-overflow (SV-COMP rules, C11 6.5.7p4) and replays under `-fsanitize=shift-base`. `bmc` refutations report nondet call sites (tagged before inlining). Local score: no-overflow 41/70, unreach-call 13/36, 0 incorrect; the same answers in BenchExec with the SV-COMP CPU/core limits (memory 12 GB: container cgroup cap). All 38 witnesses (16 correctness, 22 violation) confirmed by CPAchecker 4.2.2; UAutomizer 0.3.1 confirms 30 (3 correctness timeouts, 5 plain `x = nondet();` function_return rejections). Subset correctness witnesses are all empty (no proof there exports an invariant); no entry. **2026-09-24 (claude/invariants):** `pir` Houdini loop invariants (exported with line/column, rendered in C by the wrapper from debug info) and `bmc` modelling a canonical `__VERIFIER_assert`: no-overflow 41 → **51**/70, unreach-call 13 → **21**/36, 0 incorrect; newly true: jain_1-1, jain_2-1, jain_5-2, nested6 (pir invariants; witnesses carry 5/10/10/15 conjuncts), num_conversion_1 (bmc, no-overflow). no-overflow correctness witnesses: 31/32 validations confirmed (CPAchecker 4.2.2 + UAutomizer 0.3.1; nested6 CPAchecker timeout, UAutomizer confirms); unreach-call validation not finished |
...
| M8 | **PARTIAL** — k-induction, Houdini, contracts, bit-blaster proved; memory model proved in the semantics project; float: IEEE `+ − × ÷`, flags, FLOAT-CAST-OVF and the `--fp-checks` conditions proved against IEEE/C11 (Z3's FP theory trusted; other rounding modes, `sqrt`/`fma`/`frem` not). k-induction with a framed havoc (`KInduction.kinduction_frame_sound`, the pir step for memory-writing loops) is proved abstractly; the frame lemma for PIR memory (writes only inside the footprint, per-byte initialised flags) is not yet stated over `PrismSem/Memory.lean`. pir loop invariants (Houdini over a loop-cut encoding, docs/PIR.md "Loop invariants") implemented and soundness-tested (csmith_soundness inhouse-ptr/inhouse -n 60 and the new inhouse-loop -n 40: 0 wrong proofs, 28 proofs by invariants executed under ASan+UBSan); its soundness argument is written in the doc, not in Lean |
| M9 | **PARTIAL** — LLVM→PIR refinement for the integer fragment; freeze/undef, direct calls, stack memory, the specially translated intrinsics (incl. memcpy/memset/lifetime) and globals proved per function under a checked certificate (docs/PROOFS_REFINEMENT.md); library models, heap and pointer parameters not yet |
