...
(depth-3 regression trees; squared loss, or a censored Tobit loss where a
solver timeout only says ">= the timeout"). `tools/prism_ai/sched.py`
replays the portfolio scheduler on per-member alone-times and compares the
rules, the per-bucket history, GBDT, censored GBDT, k-NN and a winner
classifier at 1, 2, 3 and 5 cores; `tools/prism_ai/sched_e2e.py` runs the
real portfolio A/B. `tools/prism_ai/predict.py` trains, measures on a
held-out split by source file, and writes the model with `"enabled": true`
only if it beats the rules and the history by more than max(5%, run-to-run
noise) with no more timeouts (unwind: no verdict lost and less time).
`src/prism/solver/predict.cpp` loads `$PRISM_SOLVER_MODEL`, else
`<cache>/predict_model.json`, else the built-in model
(`predict_default.inc`, generated by `predict.py --emit-inc`); a model is
refused if its feature names differ, and `PRISM_SOLVER_PREDICT=0` switches
prediction off. The portfolio hook only sets the members' expected times and
the head start (never for certified requests), so a wrong prediction costs
time, never an answer. Every solve also appends a training line to
`<cache>/solve_log.jsonl`.

The built-in solver model is **on**: it beat both baselines on 599 held-out
VCs (replay) and in the real portfolio (end to end); its gain is Bitwuzla
getting the head start, which a static rule would match
(`docs/SOLVERS.md` "Learned scheduler" has the tables). The unwind model is
**off** (it lost verdicts) and `predict::unwind_for` is not wired into `pir`.

Data: `PRISM_PREDICT_COLLECT=DIR prism_tests -tc="ai-assist predict collect*"`
over the directories in `DIR/roots.txt` (default the conformance suite):
per function the verdict and time at unwind 1, 2, 4, 8, 16; per VC (up to 6
per function, unwind 8) every member alone (Z3, Bitwuzla, CaDiCaL, Kissat,
8 s timeout; ProbSAT, 0.25 s budget). A collection resumes from
`DIR/files_done.txt`.
...
| Solver scheduling | replayed portfolio, 599 held-out VCs (133 files) of tests/pir + conformance + testdata, k = 2 cores; and the real portfolio on 781 VCs, two interleaved runs | rules 17.3 s, history 15.9 s (noise between two collections 10–16%); real: rules 60.0 / 43.4 s, history 55.9 / 41.7 s | GBDT 13.0 s (−25% / −18%); real 56.7 s (one unexplained stall; 48.7 s without) / **35.9 s** (−17% / −14%, noise 0.5%). AFT = GBDT; k-NN 14.0 s; winner classifier 13.4 s; static "Bitwuzla first" 13.1 s | **on** (built-in model) |
| Unwind choice | held-out: verdict agreement with unwind 16 / seconds / BOUNDED, 525 functions | fixed 8: 99.4% / 66.2 s / 21 | GBDT 98.3% / 52.9 s / 24; k-NN 99.6% / 112.8 s / 20 | **off** (loses verdicts, or costs time) |
...
solver VCs are almost all small (median 20–30 ms per member), so the
scheduler's gain is milliseconds per VC; on hard queries the model has
