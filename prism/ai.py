...
Order:
  1. llama.cpp HTTP server (PRISM_LLAMA_SERVER, default :8080)
  2. Ollama /api/chat on qwen3.5:9b — current GGUF host on the 3090
  3. llama-cpp-python (optional) if the package imports and PRISM_GGUF loads
...
class LlamaEngine:
    ...
    def _llama_n_gpu_layers() -> int:
        ...
        if sys.platform == "win32" and not shutil.which("cl"):
            return 0
        raw = os.environ.get("PRISM_N_GPU_LAYERS", "-1")
        try:
            return int(raw)
        except ValueError:
            return -1
...
LLM_INSTALL = "ollama serve  (qwen3.5:9b) or build prism with -DPRISM_LLAMA=ON"
...
def score_prompt_seeds(seeds: list[bytes], nbytes: int) -> int:
    ...
    unique SAFE+filtered outputs. PRISM's analog is unique padded stdin
