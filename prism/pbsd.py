...
the files PRISM was asked to check. onesided_index and capacity_first
also have PRISM copies so they work on ordinary C, without FreeBSD
...
def _prism_portable(paths: list[Path], cfg: Config) -> list[Finding]:
    """PRISM copies of onesided_index / capacity_first (no FreeBSD headers)."""
    ...
    for f in run_lints(paths, root):
        ...
        out.append(Finding(
            ...
            evidence=f.evidence, extra={"via": "prism.portable"},
        ))
    return out
...
def _run_sibling_guard(
    ...
    if not script.is_file():
        ...
        return []
    try:
        spec = importlib.util.spec_from_file_location("prism_pbsd_sibling_guard", script)
