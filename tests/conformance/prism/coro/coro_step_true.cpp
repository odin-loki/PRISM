// PRISM conformance task coro/coro_step_true.cpp: expected true (no-div0)
#include <coroutine>
struct Gen {
    struct promise_type {
        int value = 0;
...
int coro_step_true(int s) {
    if (s < 1 || s > 100) return 0;
    Gen g = counter(0, s);
    g.next();
    return 100 / g.next(); /* the second value is s >= 1 */
}
