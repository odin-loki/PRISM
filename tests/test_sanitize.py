...
class TestSanitize(unittest.TestCase):
    ...
    def test_is_mingw_from_dumpmachine_ucrt(self):
        with mock.patch("helix.sanitize._dumpmachine", return_value="x86_64-w64-mingw32"):
            self.assertTrue(_is_mingw(r"C:\msys64\ucrt64\bin\gcc.exe"))
        with mock.patch("helix.sanitize._dumpmachine", return_value="x86_64-linux-gnu"):
    ...
    def test_print_file_name_echo_is_not_a_sanitizer_lib(self):
        ...
        with mock.patch("helix.sanitize._run", return_value=proc):
            self.assertFalse(_has_sanitizer_lib("gcc", _UB_FLAGS))

    def test_mingw_without_libubsan_probe_skips_compile(self):
        with mock.patch("helix.sanitize._has_sanitizer_lib", return_value=False), \
             mock.patch("helix.sanitize._compile_ok") as compile_ok:
    ...
    def test_mingw_without_libubsan_is_notrun_never_clean_or_proved(self):
        ...
        with mock.patch("helix.sanitize._find_cc", return_value=r"C:\mingw64\bin\gcc.exe"), \
             mock.patch("helix.sanitize._has_sanitizer_lib", return_value=False), \
             mock.patch("helix.sanitize._compile_and_run") as compile_and_run:
            out = run_sanitize([TD / "abs_ok.c"], Config())
    ...
    def test_flag_accept_without_ubsan_fire_is_not_supported(self):
        ...
        with mock.patch("helix.sanitize._is_mingw", return_value=False), \
             mock.patch("helix.sanitize._compile_ok", return_value=True), \
             mock.patch("helix.sanitize._run", return_value=clean):
    ...
    def test_supported_asan_clean_is_not_a_proof(self):
        with mock.patch("helix.sanitize._find_cc", return_value="/bin/gcc"), \
             mock.patch("helix.sanitize._probe_sanitizer", side_effect=lambda _cc, flags: flags[0] == "-fsanitize=address"), \
             mock.patch("helix.sanitize._compile_and_run", return_value=(laws.CLEAN, "ran under sanitizer with exit 0 (not a proof of absence)", "")):
            out = run_sanitize([TD / "abs_ok.c"], Config())
    ...
    def test_tsan_unexpected_mapping_is_notrun_not_failed(self):
        ...
        with mock.patch("helix.sanitize._find_cc", return_value="/usr/bin/clang"), \
             mock.patch("helix.sanitize._is_mingw", return_value=False), \
             mock.patch("helix.sanitize._probe_sanitizer", return_value=True), \
             mock.patch(
                 "helix.sanitize._compile_and_run",
                 return_value=(
                     laws.NOTRUN,
    ...
    def test_compile_oserror_is_notrun_never_error(self):
        with mock.patch("helix.sanitize._find_cc", return_value="/usr/bin/gcc"), \
             mock.patch("helix.sanitize._probe_sanitizer", return_value=True), \
             mock.patch("helix.sanitize._compile_and_run", return_value=(
    ...
    def test_compile_and_run_oserror_maps_to_notrun(self):
        from helix.sanitize import _compile_and_run
        with mock.patch("helix.sanitize._run", side_effect=OSError("exec format error")):
