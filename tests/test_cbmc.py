...
Helix `_run_cbmc` is law. Missing cbmc is NOTRUN (the binary did not run).
...
Mapping in helix/adapters_extra.py `_run_cbmc` (test the code, not a wish):
...
from helix.adapters_extra import _run_cbmc, run_optional_tools
...
class TestCbmcAdapter(unittest.TestCase):
    ...
    def test_verification_successful_maps_to_bounded_never_proved_unbounded(self):
        # Helix records the CBMC binary's claim through THIS mapping:
    ...
    def test_refuses_no_bounds_check_and_no_div_by_zero_check(self):
        # Weak: the refusal is still inline in `_run_cbmc` (Helix is law).
